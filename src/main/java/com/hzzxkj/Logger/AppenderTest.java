...
/**
 * 动态生成appender，并将appender加入到日志记录器中
 * @author guoxinan
 * @date 2018年10月13日
 */
...
//        ConsoleAppender consoleAppender = new ConsoleAppender();
		...
		appender.setFile(OptionHelper.substVars(
				LoggerBuilder.Path + name + "/" +  level.levelStr + ".log", context));
		...
		// 文件名格式
		String fp = OptionHelper.substVars(LoggerBuilder.Path + name + "/" + "%d{yyyy-MM-dd}" + "/"
				+ level.levelStr + "/.%d{yyyy-MM-dd}.%i.log", context);
		// 最大日志文件大小
