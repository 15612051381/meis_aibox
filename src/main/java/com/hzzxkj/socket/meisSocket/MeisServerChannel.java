 ...
 * @date      2018年10月12日
 */
public class MeisServerChannel extends ChannelInitializer<SocketChannel>{ 
	...
	public void initChannel(SocketChannel ch) throws Exception {
		...
		pipeline.addLast("CMDcoder", new MeisCmdCoder()); 
		...
		pipeline.addLast("readtime",new ReadTimeoutHandler(LoadinProperties.Tcptimeout));
		
		MeisSocketServerInHandler meisSocketServerInHandler = new MeisSocketServerInHandler(); 
		pipeline.addLast("serverhandler",meisSocketServerInHandler);
		
		MeisCmdTreatHandler meisCmdTreatHandler = new MeisCmdTreatHandler();
		pipeline.addLast("treathandler",meisCmdTreatHandler);
