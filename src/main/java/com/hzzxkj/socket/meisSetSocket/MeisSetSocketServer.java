 ...
 * @date      2018年10月9日
 */ 
public class MeisSetSocketServer {
	
	int port ;
	
	public  MeisSetSocketServer(int mPort) 
	{ 
	   this.port=mPort;
	}  
	...
	public void start()  {
		...
		try {
			...
			b.group(bossGroup, workerGroup)
					...
					.childHandler(new  MeisSetServerChannel())
					// 连接数
		    ...
		    f = b.bind(port).sync();
			if (f.isSuccess()) {
				 HzzxLogger.info("MeisSetSetSocketServer:start" + "启动Netty服务成功，端口号：" + String.valueOf(this.port), "setNettyLog"); 
				 //System.out.println("启动Netty服务成功，端口号：" + String.valueOf(this.port));  
			} 
			f.channel().closeFuture().sync();
		} catch (Exception e) {
			 HzzxLogger.error("MeisSetSetSocketServer:start" + "启动Netty服务异常，异常信息：" , e, "setNettyLog"); 
